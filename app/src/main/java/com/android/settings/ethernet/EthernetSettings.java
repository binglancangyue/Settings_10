...
public class EthernetSettings extends SettingsPreferenceFragment implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        String[] Iface = mEthManager.getAvailableInterfaces();
        if (Iface.length > 0) {
            iface = Iface[0];
            config = mEthManager.getConfiguration(iface);
    ...
    public void onRadioButtonClicked(RadioButtonPreference emiter) {
        ...
        if (emiter == mDhcpMode) {
            ...
            processIpSettings();
            if (iface != null) {
            setConfiguration(iface, mIpConfiguration);
            }
        } else {
    ...
    private void setConfiguration(String iface, IpConfiguration config) {
        ...
        Thread setConfigThread = new Thread(new Runnable() {
            public void run() {
                mEthManager.setConfiguration(iface, config);
            }
        });
